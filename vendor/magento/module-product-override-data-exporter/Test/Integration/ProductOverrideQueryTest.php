        ...
        return [
            [
                ...
                'expectedSql' => "SELECT `catalog_product_index_price`.`entity_id` AS `productId`, `catalog_product_index_price`.`customer_group_id`, SHA1(`catalog_product_index_price`.`customer_group_id`) AS `customerGroupCode`, `catalog_product_index_price`.`website_id`, `catalog_product_index_price`.`tax_class_id` AS `taxClassId`, `catalog_product_index_price`.`price` AS `minimumRegularPrice`, `catalog_product_index_price`.`final_price` AS `minimumFinalPrice`, `catalog_product_index_price`.`price` AS `maximumRegularPrice`, `catalog_product_index_price`.`final_price` AS `maximumFinalPrice`, `catalog_product_entity`.`sku`, `store_website`.`code` AS `websiteCode` "
                    ...
                    . "INNER JOIN `catalog_product_entity` ON (`catalog_product_entity`.`entity_id` = `catalog_product_index_price`.`entity_id` AND `catalog_product_entity`.`type_id` != 'configurable' AND `catalog_product_entity`.`type_id` != 'bundle') AND (catalog_product_entity.created_in <= 1 AND catalog_product_entity.updated_in > 1) "
            ...
            [
                ...
                'expectedSql' => "SELECT `catalog_category_product`.`product_id` AS `productId`, `magento_catalogpermissions_index_product`.`customer_group_id`, SHA1(`magento_catalogpermissions_index_product`.`customer_group_id`) AS `customerGroupCode`, `magento_catalogpermissions_index_product`.`grant_catalog_category_view` AS `displayable`, `magento_catalogpermissions_index_product`.`grant_catalog_product_price` AS `priceDisplayable`, `magento_catalogpermissions_index_product`.`grant_checkout_items` AS `addToCartAllowed`, `store_website`.`code` AS `websiteCode`, `catalog_product_entity`.`sku` "
                    ...
                    . "INNER JOIN `catalog_product_entity` ON (`catalog_product_entity`.`entity_id` = `catalog_category_product`.`product_id`) AND (catalog_product_entity.created_in <= 1 AND catalog_product_entity.updated_in > 1) "
                    . "WHERE ((`catalog_category_product`.`product_id` IN(::entityIds::))) "
            ...
            [
                ...
                'expectedSql' => "SELECT `magento_catalogpermissions_index_product`.`product_id` AS `productId`, `magento_catalogpermissions_index_product`.`grant_catalog_category_view` AS `displayable` "
            ...
            [
                ...
                'expectedSql' => "SELECT `configurable_product`.`entity_id` AS `productId` FROM `catalog_product_super_link` INNER JOIN `catalog_product_entity` AS `simple_product` ON (`simple_product`.`type_id` = 'simple' AND `simple_product`.`entity_id` = `catalog_product_super_link`.`product_id`) AND (simple_product.created_in <= 1 AND simple_product.updated_in > 1) INNER JOIN `catalog_product_entity` AS `configurable_product` ON (`configurable_product`.`row_id` = `catalog_product_super_link`.`parent_id`) AND (configurable_product.created_in <= 1 AND configurable_product.updated_in > 1) WHERE ((`catalog_product_super_link`.`product_id` IN(::entityIds::))) GROUP BY `configurable_product`.`entity_id`"
